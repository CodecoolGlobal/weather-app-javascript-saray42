...
async function main() {
	const codeCool = await fetch("./codecoolweather.json");
	const parsedCodeCool = await codeCool.json();
	codecoolweather = parsedCodeCool;
}
main();
	...
	if (inputField.value.length === 0) {
		favCities.map((favCity) => {
			...
			isChrome
				...
				: option.setAttribute("label", "Fav | " + favCity);
...
const autocomplete = async (input) => {
	const fetchedData = await fetch(
		apiAutocompPath + "?key=" + apiAutoCompKey + "&q=" + input
	);
...
const citySearch = async (input) => {
	const fetchedData = await fetch(
		apiWeatherPath + "?key=" + apiWeatherKey + "&q=" + input
	);
	const parsedData = await fetchedData.json();
	parsedData.error
		? displayError(parsedData)
		: displayWeatherData(parsedData);
};

const picSearch = async (input) => {
	let windwosXp = document.body.style.setProperty(
		"background",
		"url('./xp.jpg') center center"
	);
	const fetchedData = await fetch(apiPexelsPath + input, {
		headers: { Authorization: apiPexelsKey },
	});
	const parsedData = await fetchedData.json();
	parsedData.error
		? windwosXp
...
const displayCityPic = (obj) => {
	...
	body.style.setProperty(
		"background",
		`url(${obj.photos[randIndex].src.landscape}) no-repeat`
	);
...
const displayError = (error) => {
	document
		.querySelector("body")
		.style.setProperty("background", "url('./xp.jpg') center center");
...
async function showNameSuggestion() {
...
//----------------------EVENT LISTENERS----------------------//
inputField.addEventListener("input", showNameSuggestion);
...
favButton.addEventListener("click", () => {
	cities.filter((city) => {
		if (
			city.name === inputField.value &&
			!favCities.includes(inputField.value)
		) {
